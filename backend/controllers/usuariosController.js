import { listar_usuarios, criar_usuario, update_usuario, delete_usuario } from "../models/usuarios.js";
...
const listar_usuariosController = async (req, res) => {
    try{
        ...
        res.status(500).json({ mensagem: "Erro ao buscar usuarios!", error: err.message })
    }
}

const criar_usuarioController = async (req, res) => {
    try{
        const {nome, cpf, contato, sexo, cargo, vinculo, loja_vinculada} = req.body
        const data = {
            nome: nome,
            cpf: cpf,
            contato: contato,
        ...
        return res.status(200).json({mensagem: "Usuário criado com sucesso!", id: criaUsuario.insertId})
    }
    catch (err){
...
const update_usuarioController = async (req, res) => {
    try{
        ...
        return res.status(200).json({mensagem: "Usuário atualizado com sucesso!", affectedRows: atualizaUsuario.affectedRows})
    }
    catch (err){
...
export { listar_usuariosController, criar_usuarioController, update_usuarioController, delete_usuarioController }